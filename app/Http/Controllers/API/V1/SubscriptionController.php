...
class SubscriptionController extends Controller
{
    ...
    public function toggleAutorenew(Request $request, $id)
    {
        try {
            ...
            $user_id = $request->user()->id;
            $subscription = Subscription::with('subscriptionable', 'subscriptionable.cover', 'subscriptionable.owner', 'subscriptionable.prices', 'subscriptionable.prices.continent', 'subscriptionable.prices.country', 'subscriptionable.tags')->where('id', $id)->whereHas('userable', function (Builder $query) use ($user_id) {
                $query->where('user_id', $user_id);
            })->first();
